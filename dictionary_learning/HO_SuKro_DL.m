function [D,D_structured,X] = HO_SuKro_DL(params)
...
while ~found
    ...
    for k = 1:12
        grad = (D_hat*X - Y)*X.';
        if k == 10
            norm1 = norm(step*grad, 'fro');
        end
        D_hat = D_hat - step*grad;
    end
    norm2 = norm(step*grad, 'fro');
...
while ~success
    try
        ...
        for k = 1:iternum
            ...
            tol = 1e-4*norm(D,'fro');
            if k == iternum, tol = 1e-5*norm(D,'fro'); end % Better accuracy on the last iteration
...
%             Dt_old = D_hat; % for Nesterov algorithm

            % Projected Gradient Loop
            while ~converged
                % Gradient step
%                 grad = (D_hat*X - Y)*X.';
%                 temp_D_hat = D_hat - step*grad;
                
                % Gradient with Backtracking
                grad = (D_hat*X - Y)*X.';
                t = control*norm(grad,'fro')^2;
                step = step_base;
                temp_D_hat = D_hat - step_base*grad;
                obj_old = 0.5*norm(D_hat*X-Y,'fro')^2;
                while  obj_old - 0.5*norm(temp_D_hat*X-Y,'fro')^2 < step*t
                    step = shrink*step;
                    temp_D_hat = D_hat - step*grad;
                end
...
%                 temp_D_hat = D_hat;
%                 for k_grad=1:20
%                     grad = (temp_D_hat*X - Y)*X.';
%                     temp_D_hat = temp_D_hat - step*grad;
%                 end

                % Accelerate Gradient (Nesterov) with backtracking
%                 grad = (D_hat*X - Y)*X.';
%                 t = control*norm(grad,'fro')^2;
%                 step = step_base;
%                 Dt = D_hat - step_base*grad;
%                 obj_old = 0.5*norm(D_hat*X-Y,'fro')^2;
%                 while  obj_old - 0.5*norm(Dt*X-Y,'fro')^2 < step*t
%                     step = shrink*step;
%                     Dt = D_hat - step*grad;
%                 end
%                 temp_D_hat = Dt + (iter-1)/(iter+2)*(Dt - Dt_old);
%                 Dt_old = Dt;
                                
                % Projection into sum of alpha separable terms
                R_D = rearrangement_recursive(temp_D_hat,N,M);
                % Recalculating CPD from scratch
%                 Uhat = cpd(R_D,params.alpha); % supposing underlying rank (alpha) known
                ...
                temp_D_hat = rearrangement_inv_recursive(cpdgen(Uhat),N,M);
                
                % stop Criterion
                diff = norm(temp_D_hat - D_hat, 'fro');
                if diff < tol
                    converged = true;
...
%                     diff_old = diff;
                ...
                iter = iter+1;
                if iter > 1e6, error('Proj. gradient did not converge, probably due to bad parameter setting. Retrying...'),end
            end
