...
FS=filesep;
pathstr = fileparts(which('DL_image_denoise'));
...
[imout, dict, dict_unnorm, exec_times{k}] = image_denoise(params);
...
%Saving dictionary
if algo_type == 2
    dict_unnorm_reord = rearrangement_recursive(dict_unnorm,params.kro_dims.N,params.kro_dims.M);
    save(['dict_algo' num2str(algo_type) '_alpha' num2str(alpha(k))], 'dict','dict_unnorm','dict_unnorm_reord')
else
    save(['dict_algo' num2str(algo_type) '_alpha' num2str(alpha(k))], 'dict','dict_unnorm')
end
end
end
end
