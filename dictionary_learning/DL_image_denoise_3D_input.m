function DL_image_denoise_3D_input(imnum, exp_type, algo_type, iternum, sigma, mc_it,samples_training,blocksize,blocksize_m,gain)
% Same as DL_image_denoise_3D_input but receiving paramaters as an input.
...
[imout, dict, dict_struct,exec_times{k}] = image_denoise(params);
...
save(strcat('new_SNR_',num2str(params.dictsize),'atoms_algo',num2str(algo_type),'_sigma',num2str(sigma),'_',imglist(imnum).name(1:end-5),'_trainnum',num2str(params.trainnum),'_',num2str(k_mc),'_gain',num2str(round(params.gain*1000))),'params_light','alpha','SNR','exec_times');
%Saving dictionary
if save_dict
    save(['dict_algo' num2str(algo_type) '_alpha' num2str(alpha(k))], 'dict','dict_struct')
end
